...
async function handleRegister(){
  ...
  }else if(password.value.trim()!==passwordConformed.value.trim()){
...
<!--  mt:margin top-->
  <div class="flex justify-center mt-30">
    <form @submit.prevent="handleRegister" class="fieldset bg-base-200 border-base-300 rounded-box w-xs border p-4">
      ...
      <input v-model="passwordConformed" type="password" class="input" placeholder="确认密码" />

      <p v-if="errorMessage" class="text-sm text-red-500 mt-1">{{errorMessage}}</p>
      <button @click="handleRegister" class="btn btn-neutral mt-4">注册</button>
